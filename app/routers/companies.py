...
from schemas.records_schemas import Record, RecordsList, AnalyticsCompany, AnalyticsUser
...
async def request(
    ...
    company_actions_service = CompanyActionsService(current_user=current_user, db=db)
...
async def company_quizzes(
    company_id: int,
    params: Params = Depends(),
    current_user: User = Depends(get_user),
    db: Database = Depends(get_db),
) -> Page:
    company_actions_service = CompanyService(current_user=current_user, db=db)
    quizzes = await company_actions_service.list_quizzes(
...
async def members_records(
    ...
    records = await company_actions_service.get_members_records(
        user_id=user_id, quizz_id=quizz_id, redis=redis
    )
    if csv:
...
@router.get("/{company_id}/analytics/averages", response_model=AnalyticsCompany)
async def users_analytics_averages(
    company_id: int,
    current_user: User = Depends(get_user),
    db: Database = Depends(get_db),
) -> AnalyticsCompany:
...
@router.get("/{company_id}/analytics/", response_model=AnalyticsCompany | AnalyticsUser)
