...
class QuizzService(CompanyService):
    ...
    async def create_quizz(self, company_id: int, quizz_data: QuizzCreate) -> Quizz:
        ...
        for question_numb, question in enumerate(quizz_data.questions):
            await self.create_questions(question=question, quizz_id=quizz_id, question_numb=question_numb)
    ...
    async def pass_quizz(self, company_id: int, quizz_id: int, quizz_data: QuizzData) -> Record:
