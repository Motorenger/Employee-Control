...
class CompanyActionsService(CompanyService):
    ...
    async def get_invites(self) -> InvitesList:
        ...
        query = self.invites.select().where(
            self.invites.c.company_id == self.company_id
        )
    ...
    async def get_members(self) -> UserList:
        ...
        query = (
            users.select()
            .join(self.company_members)
            .where(self.company_members.c.company_id == self.company_id)
    ...
    async def get_requests(self) -> RequestList:
        ...
        query = self.requests.select().where(
            self.requests.c.company_id == self.company_id
        )
    ...
    async def send_request(self, request_data: RequestData):
        ...
        if company.owner_id == self.current_user.id:
            raise HTTPException(
                status_code=400, detail="User is already a member of the company"
            )
        query = (
            users.select()
            .join(self.company_members)
            .where(self.company_members.c.company_id == request_data.company_id)
            .where(users.c.id == self.current_user.id)
        )
    ...
    async def delete_member(self, user_id: int):
        ...
        query = self.company_members.delete().where(
            self.company_members.c.user_id == user_id
        )
    ...
    async def admin(self, user_id: int):
        ...
        query = (
            self.company_members.update()
            .where(self.company_members.c.user_id == user_id)
            .values({"admin": True})
        )
        await self.db.execute(query)
    ...
    async def get_admins(self) -> UserList:
        ...
        query = (
            ...
            .join(self.company_members)
            .where(self.company_members.c.company_id == self.company_id)
            .where(self.company_members.c.admin == True)
            .where(users.c.id == self.company_members.c.user_id)
        )
    ...
    async def get_analytics_users(self, user_id: int | None) -> AnalyticsCompany | AnalyticsUser:
        ...
        if user_id:
            query = (
                self.records.select()
                .where(
                    self.records.c.company_id == self.company_id,
                    self.records.c.user_id == user_id,
                )
            ...
            query = (
                self.records.select()
                .where(self.records.c.company_id == self.company_id)
                .order_by(self.records.c.user_id)
