...
class CompanyService:
    ...
    async def check_for_existing(
        ...
        if check_owner_admin:
            admin = await self.db.fetch_one(
                query=self.company_members.select()
    ...
    async def get_companies(self) -> CompanyList:
        query = self.companies.select().where(
            (self.companies.c.visible == True)
            | (self.companies.c.owner_id == self.current_user.id)
        )
    ...
    async def create_company(self, company: CompanyBase) -> Company:
        if (
            await self.check_for_existing(comp_name=company.name, exeptions=False)
            is not None
        ):
            raise HTTPException(
                status_code=400, detail="Such company name already exists"
            )
    ...
    async def update_company(self, company_id: int, company_data: str) -> Company:
        ...
        query = (
            ...
            .where(self.companies.c.id == company_id)
            .values(**update_data)
        )
